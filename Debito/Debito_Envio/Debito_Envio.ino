...
while(Serial.available()){
  if(Serial.available()>0){
    Pacote[counter_receber] = Serial.read();
    counter_receber++;
  }
...
memset(Packet,'\0', 30);
radio.stopListening();
*Packet = CreatePacket(Send_Dif, string_size, Aux_Array, posArray,Sent_Packets);
Send_Dif = false;
...
byte CreatePacket(bool SendDif, int StringSize, char* Text,int newpos,int Sent_Packets){
