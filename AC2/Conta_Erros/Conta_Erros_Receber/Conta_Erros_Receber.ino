...
void CheckPacket(){
  if(SequenceBit == Prev_SequenceBit && !Waiting_Retrans){ //É introduzido este caso visto que o emissor apenas muda de sequence bit caso haja um ACK
    //Serial.println("TIMEOUT DA CONFIRMAÇÃO");// Logo este if serve como maneira de contabilizar os casos em que houve a perda de um ACK visto que o Receptor não estava à espera de uma retransmissão nem do mesmo sequenceBit
    NTramas_Retrans++; // Visto que este tinha enviado um ACK
   }
